...
const categories = ["Totes", "Cultura", "Urbanisme", "Història", "Activitats"];
...
export default function NewsPortal() {
  ...
  const filteredArticles = useMemo(() => {
    return articlesData.filter((article) => {
      const matchesCategory = selectedCategory === "Totes" || article.category === selectedCategory;
    ...
    <div>
      ...
      {paginatedArticles.length > 0 ? (
        ...
        <div className="text-center py-20 bg-surface-container-low rounded-2xl border border-outline-variant/20">
          ...
          <p className="font-body-lg text-on-surface-variant">No s'han trobat notícies que coincideixin amb la cerca.</p>
        </div>
      )}
